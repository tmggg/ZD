...
namespace EasyNetVars
{
    ...
    public class NetVars
    {
		...
		public ArrayList ReadValues()
        {
            ...
            while (((receiveBytes[8] != cobID) | receiveBytes == null | waitForNextElement | elementZeroDetected == false | returnList.Count != dataType.Count))
            {
                ...
                if (cTelegramReceive.Items != dataType.Count)
                {
                    ArrayList returnListError = new ArrayList();
                    returnListError.Add(cTelegramReceive.Items);
                    return returnListError;
                }
        ...
        public void SendValues()
        {
            ...
            //s.SendBufferSize = 81920;
            //int temp = s.SendBufferSize;
            ...
            for (int k = 0; k < numberOfDatagrams; k++)
            {
                ...
                {
                    switch (dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].DataTypes)
                    {
                        case DataTypes.booltype:
                            Array.Resize(ref send_buffer, send_buffer.Length + 1);
                            send_buffer[20 + byteCount] = Convert.ToByte(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                            byteCount++;
                            break;
                        ...
                        case DataTypes.wordtype:
                            ...
                            uint16array[0] = Convert.ToUInt16(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                        ...
                        case DataTypes.dwordtype:
                            ...
                            uintarray[0] = Convert.ToUInt32(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                            Buffer.BlockCopy(uintarray, 0, send_buffer, 20 + byteCount, 4);
                            byteCount = byteCount + 4;
                            break;
                        case DataTypes.sinttype:
                            Array.Resize(ref send_buffer, send_buffer.Length + 1);
                            sbytearray[0] = Convert.ToSByte(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                        ...
                        case DataTypes.inttype:
                            Array.Resize(ref send_buffer, send_buffer.Length + 2);
                            int16array[0] = Convert.ToInt16(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                        ...
                        case DataTypes.dinttype:
                            Array.Resize(ref send_buffer, send_buffer.Length + 4);
                            intarray[0] = Convert.ToInt32(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                        ...
                        case DataTypes.realtype:
                            Array.Resize(ref send_buffer, send_buffer.Length + 4);
                            floatarray[0] = (float)Convert.ToDouble(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                        ...
                        case DataTypes.lrealtype:
                            ...
                            doublearray[0] = Convert.ToDouble(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue);
                            Buffer.BlockCopy(doublearray, 0, send_buffer, 20 + byteCount, 8);
                            byteCount = byteCount + 8;
                            break;
                        case DataTypes.stringtype:
                            char[] charArray = new char[dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].FieldLength + 1];
                            byte[] byteArray = new byte[dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].FieldLength + 1];
                            charArray = Convert.ToString(dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].SendValue).ToCharArray();
                            int oldLength = charArray.Length;

                            if (charArray.Length <= dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].FieldLength)
                                Array.Resize(ref charArray, (dataType[i + fixsum + this.CountNumberOfDatagrams(tagCounter)].FieldLength + 1));
                ...
                fixsum = tagCounter - CountNumberOfDatagrams(tagCounter) - 1;
                ...
                datagramCounter++;
                Console.WriteLine("Send Buffer Size {0}", send_buffer.Length);
            }
        }
    ...
    public class CDataTypeCollection
    {
        ...
        public CDataTypeCollection(object value, DataTypes dataTypes, string dataName)
        {
            this.dataTypes = dataTypes;
            this.sendValue = value;
